...
#     binary_mask = (get_highlight_mask(input_scene,threshold=threshold,luminance_mode=luminance_mode) < 0.7).to("cpu", torch.bool)
...
#         pred_scene = pred_scene.unsqueeze(0).to(input_scene.device, torch.float32)
...
def blend_light_source(input_scene, pred_scene,threshold=0.99,luminance_mode=False):
    binary_mask = (get_highlight_mask(input_scene,threshold=threshold,luminance_mode=luminance_mode) > 0.9).to("cpu", torch.bool)
    # binary_mask = semantic_mask
    binary_mask = binary_mask.squeeze(0)
    binary_mask = binary_mask[0,:,:]  # (h, w)
    ...
    if kernel_size > 0:
        ...
        print("bbbb")
    return blend
def blend_background(input_scene, pred_scene,threshold=0.3,luminance_mode=False):
    binary_mask = (get_highlight_mask(input_scene,threshold=threshold,luminance_mode=luminance_mode) < 0.9).to("cpu", torch.bool)
    # binary_mask = semantic_mask
    binary_mask = binary_mask.squeeze(0)
    binary_mask = binary_mask[0,:,:]  # (h, w)
    ...
    if kernel_size > 0:
        ...
        blend = input_scene * (1 - mask_rgb) + pred_scene *  mask_rgb
        print("aaaa")
